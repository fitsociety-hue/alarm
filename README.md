...
1. **자동화된 모니터링**:
    - 지정된 URL(강동어울림복지관 이용상담문의 게시판)을 주기적으로 확인
    - `BeautifulSoup`을 사용하여 HTML 구조 파싱
2. **웹 대시보드**:
    - `docs/` 리소스를 활용한 GitHub Pages 웹 대시보드 제공
    - 프론트엔드 환경에서 하루 n번의 알림 시간 스케줄을 손쉽게 설정 및 수정(KST 기준)
3. **새 글 감지**:
    - 이전에 확인한 게시글 ID와 비교하여 새로운 글만 식별
    - 30일 이상 오래된 글 제외, 현재 연도(예: 2026년) 이외의 과거 게시물 감지 제외
4. **Google Chat 연동**:
    - Webhook 설정으로 챗 스페이스에 알림 전송
    - 제목, 작성자, 날짜 등을 포함한 카드(Card) 형태의 깔끔한 알림 메시지
5. **자동 실행**:
